...
variable "project_name" {
  ...
  type        = string
  default     = "liquid-metal-acceptance-tests"
}
...
# Create device for dhcp, nat routing, management cluster etc
resource "metal_device" "management_cluster" {
...
resource "metal_port" "bond0_dhcp" {
  port_id  = [for p in metal_device.management_cluster.ports : p.id if p.name == "bond0"][0]
  layer2   = false
  bonded   = true
...
resource "null_resource" "setup_management_cluster" {
  connection {
    type        = "ssh"
    host        = metal_device.management_cluster.network.0.address
    user        = "root"
    port        = 22
